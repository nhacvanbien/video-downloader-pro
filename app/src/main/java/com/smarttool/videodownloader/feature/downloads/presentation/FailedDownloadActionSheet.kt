...
/** Sheet for a failed download row — the design's "Retry / Delete" pair. */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun FailedDownloadActionSheet(
    onRetry: () -> Unit,
    onDelete: () -> Unit,
    onDismiss: () -> Unit,
) {
    ModalBottomSheet(
        ...
        AppLocaleProvider {
            Column(modifier = Modifier.padding(bottom = 24.dp)) {
                ActionRow(R.drawable.ic_reload, R.string.string_retry_download, TextColor, onRetry)
